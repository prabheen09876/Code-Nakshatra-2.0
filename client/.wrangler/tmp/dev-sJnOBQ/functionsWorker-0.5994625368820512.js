...
// .wrangler/tmp/bundle-oi9vjT/checked-fetch.js
...
// .wrangler/tmp/pages-Qk4PIF/functionsWorker-0.5994625368820512.mjs
...
var deliverables = sqliteTable("deliverables", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var changeRequests = sqliteTable("change_requests", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  projectId: integer("projectId").references(() => projects.id).notNull(),
...
var authMiddleware = /* @__PURE__ */ __name2(async (c, next) => {
  ...
  if (cfJwt) {
    try {
      ...
      if (!email) {
        return c.json({ success: false, message: "Invalid CF Access token: no email" }, 401);
      }
      ...
      if (!user) {
        const [newUser] = await db.insert(users).values({
          ...
          email,
          password: "@@cf_access@@",
          // No password needed – CF handles auth
          role: "client"
      ...
      return c.json({ success: false, message: "CF Access token decode failed" }, 401);
    }
  }
  const localEmail = c.env.LOCAL_DEV_EMAIL;
  if (localEmail) {
    let user = await db.query.users.findFirst({
      where: eq(users.email, localEmail)
    });
    if (!user) {
      const [newUser] = await db.insert(users).values({
        name: c.env.LOCAL_DEV_NAME || "Dev User",
        email: localEmail,
        password: "@@local_dev@@",
        role: c.env.LOCAL_DEV_ROLE || "client"
      }).returning();
      user = newUser;
    }
    c.set("userId", user.id);
...
// .wrangler/tmp/bundle-oi9vjT/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-oi9vjT/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5994625368820512.js.map
